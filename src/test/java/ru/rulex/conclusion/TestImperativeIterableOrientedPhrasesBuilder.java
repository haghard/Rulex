...
public class TestImperativeIterableOrientedPhrasesBuilder {
  ...
  private final ImmutableList<Model> list = ImmutableList.of(
      Model.values(121), Model.values(122));
  ...
  public void testImperativeExecutionPhrasesBuilderWithMock() {
    ...
    when(delegateMock.execute(Mockito.<Integer> anyVararg())).thenAnswer(
        new Answer<Object>() {
          @Override
    ...
    AbstractIterableOrientedPhrasesBuilder builder = 
      ...
      protected void build() {
        through(Model.class, Integer.class, "testImperativeExecutionPhrasesBuilderWithMock")
          .lambda(DelegateFactory.from(1, 7, 13))
  ...
  public void testSimpleImperativePhrasesBuilder() {
    ...
    final ConclusionPredicate<Delegate<FluentConclusionPredicate>> compositionPredicate = 
        DelegateFactory.<FluentConclusionPredicate> from(FluentConclusionPredicate.never());
    ...
    final Delegate<FluentConclusionPredicate> targetDelegate = new Delegate<FluentConclusionPredicate>() {
      ...
      public boolean execute(FluentConclusionPredicate... arguments) {
        assertSame("testSimpleImperativePhrasesBuilder thread error ",
    ...
    AbstractIterableOrientedPhrasesBuilder builder = 
      ...
      @Override
      protected void build() {
        through(Model.class, FluentConclusionPredicate.class, "never()")
          .lambda(compositionPredicate)
          .delegate(targetDelegate);
      }
    };

    try {
      assertFalse("testSimpleImperativePhrasesBuilder error !!!",
          builder.async(list).checkedGet());
    } catch (Exception ex) {
  ...
  public void testComplexImperativePhrasesBuilder() {
    // less 123 and eq 122
    final ConclusionPredicate<Delegate<FluentConclusionPredicate>> fluentPredicates = 
        DelegateFactory.<FluentConclusionPredicate> from(
            fluent().less(argument(123), descriptor(Model.class, Model.INT_ACCESSOR))
        	.and(
    ...
    final Delegate<FluentConclusionPredicate> targetDelegate = new Delegate<FluentConclusionPredicate>() {
      ...
      public boolean execute(FluentConclusionPredicate... fluentPredicate) {
        Preconditions.checkArgument(fluentPredicate[0] != null,
            "null FluentConclusionPredicate");
    ...
    AbstractIterableOrientedPhrasesBuilder builder2 = new ImperativeIterableOrientedPhrasesBuilder() {
      ...
      protected void build() {
        through(Model.class, FluentConclusionPredicate.class,"testComplexImperativePhrasesBuilder")
          .lambda(fluentPredicates)
          .delegate(targetDelegate);
      }
    };

    // eq 123 or eq 124
    final ConclusionPredicate<Delegate<FluentConclusionPredicate>> fluentPredicates0 = 
        DelegateFactory.<FluentConclusionPredicate> from(
            fluent().eq(argument(123), descriptor(Model.class, Model.INT_ACCESSOR))
        	.or(
    ...
    AbstractIterableOrientedPhrasesBuilder builder3 = new ImperativeIterableOrientedPhrasesBuilder() {
      @Override
      protected void build() {
        through(Model.class, FluentConclusionPredicate.class, "testImperativeExecutionPhrasesBuilder")
          .lambda(fluentPredicates0)
          .delegate(targetDelegate);
      }
    };
    try {
      assertTrue("testComplexImperativePhrasesBuilder-1 error !!!",
          builder2.async(list).checkedGet());
      assertFalse("testComplexImperativePhrasesBuilder-2 error !!!",
          builder3.async(list).checkedGet());
    } catch (Exception ex) {
  ...
  public void testImperativePhrasesBuilderWithPStrategy() {
    ...
    final Delegate<FluentConclusionPredicate> targetDelegate = new Delegate<FluentConclusionPredicate>() {
      ...
      @Override
      public boolean execute(FluentConclusionPredicate... arguments) {
        assertNotSame("testImperativePhrasesBuilderWithPStrategy error",
            Thread.currentThread(), currentThread);
        return Optional.fromNullable(arguments[0]).or(always()).apply(1);
      }
    };
    try {
      ...
      assertFalse(
          "testImperativePhrasesBuilderWithPStrategy error !!!",
          new ImperativeIterableOrientedPhrasesBuilder() {
            @Override
            protected void build() {
              rule(ParallelStrategy.<Boolean, PhraseExecutionException> separateThreadStrategy(),
                  Model.class, FluentConclusionPredicate.class, "builder")
                  .lambda(compositionPredicate).delegate(targetDelegate);
            }
  ...
  public void testStartContextAndEndContextInImperativeWay() {
    final ImmutableList<Model> list = ImmutableList.of(
        Model.values(4), Model.values(6), Model.values(6), Model.values(55));
    ...
    final Delegate<FluentConclusionPredicate> targetDelegate = new Delegate<FluentConclusionPredicate>() {
      ...
      @Override public void setContent(Iterable<?> iterable) {
        this.list0 = ImmutableList.copyOf(iterable);
      }
      
      @SuppressWarnings("unchecked")
      @Override public boolean execute(FluentConclusionPredicate... arguments) {
        assertTrue(list0.size() == 4);
        assertTrue(arguments.length == 2);
        return arguments[0].apply(list0.get(0)) && 
            arguments[1].apply(list0.get(list0.size()-1));
      }
    };
    ...
    try {
      assertTrue("testStartContextAndEndContextInImperativeWay error !!!",
        new ImperativeIterableOrientedPhrasesBuilder() {
          @Override
          protected void build() {
            through(Model.class, FluentConclusionPredicate.class, "ImperativeBuilder")
                .lambda(startEndPredicates)
                .delegate(targetDelegate);
          }
        }.async(list).checkedGet());
    } catch (Exception ex) {
