...
public class GuiceEventOrientedEngineModuleTest {
  ...
  public void testSelectorPredicateApi() throws Exception {
    ...
    toPredicate(callOn(Delegate.class).execute(new Integer[] {1,2,3})).apply(new Delegate<Integer>() {
      @Override
  ...
  /**
   * test or condition on single event fields through {@code Phrases.SINGLE_ANY_TRUE} 
   */
  @Test
  public void testAnyGuiceModuleWithSelectors() {
    Injector injector = createInjector(
        $expression(Phrases.ANY_TRUE,
          $more(92, intSelector, "92 > en.getInput()")),
      ...
      $expression(
          $more(8f, floatSelector, "8 > en.getFloat()"), 
            $or("or test condition",
              $more(7, intSelector, "7 > en.getInteger()"),
