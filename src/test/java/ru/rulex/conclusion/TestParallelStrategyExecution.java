...
public class TestParallelStrategyExecution {
  ...
  FutureMerger<Boolean> allMerger = new FutureMerger<Boolean>() {
    @Override
    public ListenableFuture<List<Boolean>> merged(
  ...
  public void testBind() throws Exception {
    ...
    assertTrue("testBind error !!!",
        Callables.bind(Callables.unit(en),
            new ConclusionFunction<Model, Callable<Boolean>>() {
              @Override
  ...
  public void testFmap() throws Exception {
    Model en = Model.values(40);
    assertTrue("testFmap error !!!",
        Callables.fmap(new ConclusionFunction<Model, Boolean>() {
          @Override
  ...
  @Test
  public void testListenableFutureWithParallelStrategy()
    ...
    ImmutableList.Builder<ListenableFuture<Boolean>> blist = ImmutableList
        .builder();
    ...
    assertTrue("testWithParallelStrategy error !!!",
  ...
  public void testObtainWithException() throws Exception {
    ...
    try {
      Callables.bind(
          Callables.obtain(pStrategy.lift(
              new ConclusionFunction<Model, Boolean>() {
                @Override
                public Boolean apply(Model argument) {
                  throw new IllegalArgumentException("expected");
                }
              }).apply(en)),
          new ConclusionFunction<Boolean, Callable<Integer>>() {
            @Override
