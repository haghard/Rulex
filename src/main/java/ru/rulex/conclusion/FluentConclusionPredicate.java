 ...
 * 
 * <p>
 * Static factory methods are a convenient way to simulate named parameters in Java,
 * so methods {@code argument(T value)} and {@code descriptor()} is a named parameters.
 * See article on <a href="http://www.infoq.com/articles/internal-dsls-java"> DSL </a>. 
 * </p>
 * 
 */
public abstract class FluentConclusionPredicate<T> implements
    ConclusionPredicate<T> {

  private static final Logger logger = Logger.getLogger(FluentConclusionPredicate.class);
  
  private final static Map<Class<?>, JvmBasedLanguageAdapter> languageAdaptors = 
      new HashMap<Class<?>, JvmBasedLanguageAdapter>();
  ...
  public static void registerLanguageAdaptor(Class<?> functionClass, JvmBasedLanguageAdapter adaptor) {
  ...
  /**
   * Generic singleton factory pattern (Effective java 2e ITEM 27:)
   * On occasion, you will need to create an object that is immutable but applicable to many different types.
   * Because generics are implemented by erasure, you can use a single
    ...
    FLUENT {
      ...
      public boolean apply(Object argument) {
        throw new IllegalStateException(
            "FluentConclusionPredicate illegal state !!!");
      }
    },
    ALWAYS {
      @Override
  ...
  /**
   * Method for define a start point for
   * fluent building process.
   *
   * <p>
   * since {@code fluent()} return FluentConclusionPredicate<T> instance
   * we able to call his method {@code apply()} which
  ...
  public static <T> FluentConclusionPredicate<T> bind(
    ...
    return new FluentConclusionPredicate<T>() {
      @Override public boolean apply(T argument) {
  ...
  public FluentConclusionPredicate<T> and(ConclusionPredicate<? super T> predicate) {
    return this == ENTRY_POINT ?
      (predicate == ENTRY_POINT ? bind(ConstantPredicate.NEVER.<T>withNarrowType()) : bind(predicate)) :
      (predicate == ENTRY_POINT) ? bind(this) : DslFacade.and(this, predicate);
  }
  ...
  public FluentConclusionPredicate<T> or(ConclusionPredicate<? super T> predicate) {
    return this == ENTRY_POINT ?
      (predicate == ENTRY_POINT ? bind(ConstantPredicate.NEVER.<T>withNarrowType()) : bind(predicate)):
      (predicate == ENTRY_POINT) ? bind(this) : DslFacade.or(this, predicate);
  }
   ...
   * @return
   */
  public static <T> TypeSafeSelector<T, String> string(
   ...
   */
  public static <T> ConclusionPredicate<T> lambda(
      ConclusionPredicate<T> conclusionPredicate) {
  ...
  private static class TypeSafeSelectorPredicate<T, E> implements ConclusionPredicate<T> {
    ...
    private TypeSafeSelectorPredicate(ConclusionPredicate<E> predicate,
                          TypeSafeSelector<T, E> path) {
   ...
   * @param <E>
   */
  public static class SelectorPredicate<T, E> implements
      ConclusionPredicate<T> {
    private final ConclusionPredicate<E> predicate;
    private final Selector<T, E> selector;

    public SelectorPredicate(ConclusionPredicate<E> predicate,
   ...
   * @param <T>
   */
  static final class OrConclusionPredicate<T> extends
      FluentConclusionPredicate<T> {
    private final Iterator<ConclusionPredicate<? super T>> predicates;

    OrConclusionPredicate(
        Iterator<ConclusionPredicate<? super T>> predicates0) {
   ...
   * @param <T>
   */
  static class AndConclusionPredicate<T> extends
      FluentConclusionPredicate<T> {
    private T argument;
    private final Iterator<ConclusionPredicate<? super T>> predicates;

    AndConclusionPredicate(
        Iterator<ConclusionPredicate<? super T>> predicates) {
  ...
  /**
   * Returns the composition of TypeSafeSelector and ConclusionPredicate. For every
   * {@code argument}, the generated arguments returns
  ...
  private static final class EqualsConclusionPredicates<T extends Number & Comparable<? super T>>
    ...
    @Override
    public String toString() {
      return MessageFormat.format("{0} ({1} $equals {2}) => {3} ",
          "EqualsConclusionPredicate", value, argument != null ? argument
          : "placeholder", result);
    }
  }
  ...
  private static final class MoreConclusionPredicate<T extends Number & Comparable<? super T>>
    ...
    @Override
    public String toString() {
      return MessageFormat.format("{0} ({1} $more {2}) => {3} ",
          "MoreConclusionPredicate", value, argument != null ? argument
          : "placeholder", result);
    }
  }
  ...
  private static final class LessConclusionPredicate<T extends Number & Comparable<? super T>>
    ...
    @Override
    public String toString() {
      return MessageFormat.format("{0} ({1} $less {2}) => {3} ",
          "LessConclusionPredicate", value, argument != null ? argument
          : "placeholder", result);
    }
  }
  ...
  public static <T> ConclusionPredicate<T> toJavaPredicate(final Object predicate) {
    ...
    return new ConclusionPredicate<T>() {
      @Override
      public boolean apply(Object argument) {
        return (boolean) la.call(predicate, new Object[] {argument});
      }
    };
  }

  public static <E, U extends Comparable<? super U>> Selector<E, U> toJavaSelector(final Object selector) {
    if (selector == null) 
